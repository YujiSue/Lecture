...
# 課題レポートの提出
  ...
  保存後はメールに添付して送信してください。  
  
  <span style="color: red;">※保存がうまくいかないときは</span>
  
  webブラウザのバージョン依存性が出るケース、スマートフォンからだと上手く作動しないケースがあるようです。  
  ...
  基本的にこのipynbのファイルはweb上で開くのが前提のものですので、ダウンロード後に見れないかと思います。  
  基本的にダウンロードが上手くいって、自分の機器上に保存されれば、内容はwebで表示されているものと変わりませんので、安心して送信してください。  
  なお、プログラミングに興味が出て自分のPC上で閲覧・編集したい場合、いくつかやり方はありますが[Visual Studuio Code](https://code.visualstudio.com/)というソフトをダウンロードしてインストールし、追加でPythonの機能拡張を入れるのが比較的簡単です。  
  ただし、メモリやストレージを圧迫する可能性もあるので、原則はweb上で操作し、最後にダウンロード・送信をお勧めします。
